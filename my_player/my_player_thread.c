...
typedef struct VideoState
{
  ...
  int audio_pkt_size;
  struct SwrContext *audio_swr_ctx; //重采样上下文 重采样是为了符合音频设备的参数才能正常播放
  ...
  int pictq_size, pictq_rindex, pictq_windex;   //队列相关信息
...
void video_refresh_timer(void *userdata)
{
  ...
  if (is->video_st)
  {
    ...
    else
    {
      ...
      */
      schedule_refresh(is, VIDEO_REFRESH_TIME);
